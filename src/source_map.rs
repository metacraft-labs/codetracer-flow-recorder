//! Source mapping for Cadence programs.
//!
//! Provides byte-offset to line-number mapping for Cadence source files,
//! used to map execution locations back to source code lines.

use std::path::Path;

/// Maps byte offsets in a Cadence source file to line numbers.
///
...
mod tests {
    ...
    fn test_byte_to_line_simple() {
        let source = "line1\nline2\nline3\n";
        let map = SourceMap::from_source(&PathBuf::from("test.cdc"), source);
