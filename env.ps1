# codetracer-flow-recorder Windows dev environment (PowerShell)
# Usage: . .\env.ps1
#
...
#      MSVC, and Go).  These are provisioned by the main `codetracer` repo's
#      env.ps1, which this script dot-sources.  Nim is needed because the
#      `codetracer_trace_writer_nim` crate's build script compiles a Nim
#      static library; Go is needed because this recorder's `build.rs`
#      compiles the `go-helper/` Cadence trace helper (`go build`).
#
...
#      `link.exe` bypasses PATH resolution entirely.
...
# bootstrap steps so activation is fast.  Go is *not* skipped -- the Cadence
# helper build needs it.
...
Write-Host "codetracer-flow-recorder dev environment ready."